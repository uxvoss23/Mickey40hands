...
## Recent Changes
- **2026-02-17**: Route History auto-cleanup: completed routes are deleted from DB and removed from history after marking complete. Routes with all stops cancelled are also auto-deleted. Cancel button in Route History now navigates to Route Planner (map view) and opens the cancellation modal. On load, completed routes and routes with zero active stops are filtered out for decluttering.
